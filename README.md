...
- Commits automatically when a [pi-goal](https://github.com/earendil-works/pi-goal) completes (`on_goal` trigger, opt-in). Other modes available.
...
With `enabled = true` and `trigger_mode = "on_goal"` (default), the extension hooks into [pi-goal](https://github.com/earendil-works/pi-goal)'s lifecycle. When a goal managed by pi-goal transitions to **completed** status, pi-committer automatically stages and commits the changes:
