...
cd ~/projects/pi-committer/extensions/pi-committer && npm install

# Load it in a session (no global interference)
pi -e ~/projects/pi-committer/extensions/pi-committer/index.ts
```

### Global auto-discovery

```bash
ln -s ~/projects/pi-committer/extensions/pi-committer ~/.pi/agent/extensions/pi-committer
```
...
```bash
git clone <this-repo>
cd pi-committer/extensions/pi-committer
npm install
